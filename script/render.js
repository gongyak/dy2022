...
const renderTitle = (selected_tag) => {
    ...
    if (selected_tag.trim() === "") {
        title = `&#8220;<span class="justbold">경기도민의</span> <span>[&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;]</span><span class="justbold">를 위한 약속</span>&#8221;` 
    } else {
        var header = "나의"
...
const selectTag = (tag) => {
    ...
    gtag('event', 'dy2022:카테고리 선택', {
        'event_category': '카테고리',
        'event_lavel': `${tag}`
    })
    gtag('event', `dy2022:카테고리: ${tag}`, {
        'event_category': '카테고리',
        'event_lavel': `${tag}`
    })
}
...
const expandPledge = (index) => {
    ...
    if (element_title || element_location) {
        ...
        gtag('event', 'dy2022:세부공약 선택', {
            'event_category': '세부공약',
            'event_lavel': `${title} ${location}`
        })
        gtag('event', `dy2022:세부공약: ${title} ${location}`, {
            'event_category': '세부공약',
            'event_lavel': `${title} ${location}`
        })
...
const selectSection = (section) => {
    ...
    switch (section) {
        case 0:
            ...
            gtag('event', 'dy2022:섹션전환', {
                'event_category': '섹션',
                'event_lavel': `관심분야`
            })
            gtag('event', 'dy2022:섹션전환: 관심분야', {
                'event_category': '섹션',
                'event_lavel': `관심분야`
            })
